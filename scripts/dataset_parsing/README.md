...
## 1. Curation and parsing of clinical data
### 1.1. Parsing of AMP-AD datasets
#### 1.1.1. Parsing of AMP-AD individual metadata
...
#### 1.1.2. Homogenizing RNAseq harmonization metadata and counts matrices
...
#### 1.1.3. Homogenizing LBP metadata.
...
### 1.3. Merge GTEx, LBP and RNAseq Harmonization datasets
...
### 1.4. Obtention of TBI data and merge with the combined dataset
...
### 1.5. Obtention of BrainSeq Phase 2 data
...
### 1.8. Integrate all clinical samples into a single file and pre-process
#### 1.8.1 Add brainSeq Phase II, AgeAnno and perturbation compilation to merged dataset
...
#### 1.8.3 Divide samples in train and test sets
Here, samples are divided in train and test subsets in a 2:1 proportion. This step is performed by `train_test_split.R`. We do this here, instead of in [model_training](./scripts/model_training/) module for 2 reasons:
- Knowing the train and test partition before normalization allows us to perform quantile-normalization only on the test set, and then apply the training distribution on the rest of the samples, preventing data leakage.
